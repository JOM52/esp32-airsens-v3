...
# def get_mac():
#     return = hexlify(unique_id(),':').decode().upper()
...
#             msg = 'jmb,'  + conf.SENSOR_LOCATION + sensor_cpl + ',' + sensor_actif +',' 
        ...
        sta = WLAN(STA_IF)
        sta.active(True)
        print('sta.config("channel"):', sta.config('channel'))
...
#         print('espnow.get_peer(host_mac_adress):',espnow.get_peer(host_mac_adress))
...
#             msg_received_by_host = espnow.send(None, msg, True)
            if not msg_received_by_host:
                log.counters('lost', True)
#         print(hexlify(host_mac_adress).decode())
#         espnow.send(host_mac_adress, ' ', True)
        if conf.TO_PRINT: print()
