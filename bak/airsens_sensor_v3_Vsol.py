...
v3.1.2 : 24.06.2023 --> added bat in the main measurement loop so AVERAGING_BAT no mure needed
"""
...
def main():
    ...
    try:
        ...
        for i, sensor_type in enumerate(conf.SENSORS):
            ...
            sensor_id = sensor_mac + '.' + str(i)
            msg = 'jmb,'  + sensor_id + ',' + conf.SENSOR_LOCATION + ',' + sensor_type + ','
            ...
            for measurement in measurement_list:
                ...
                for l in range(conf.AVERAGING_BME):
                    if measurement == 'temp':
                        mes_val = float(sensor.temperature)
                    elif measurement == 'pres':
                        mes_val = float(sensor.pressure)
                    elif measurement == 'hum':
                        mes_val = float(sensor.humidity)
                    elif measurement == 'gas':
                        mes_val = float(sensor.gas)
                    elif measurement == 'alt':
                        mes_val = float(sensor.altitude)
                    elif measurement == 'sol':
                        mes_val = float(sol.read() * (2 / 4095) / conf.DIV)
                    elif measurement == 'bat':
                        mes_val = float(pot.read() * (2 / 4095) / conf.DIV)
                    mean_val += mes_val / conf.AVERAGING_BME
                if measurement == 'bat':
                    bat = mean_val
                msg += measurement + ':' + str(mean_val) + ';'
